...
function loadEntities() {
    ...
    fetch(`https://${CA_RESOURCE}/ca_getEntities`, {
        ...
        if (!t || !t.trim() || t.trim() === 'null' || t.trim() === 'false') {
            panel.innerHTML = _entHeader() + '<div class="empty-state"><div class="empty-icon">'+icon('warn')+'</div><div class="empty-text">No response — ensure resource is loaded</div></div>';
            return;
        }
        const data = JSON.parse(t);
        renderEntities(panel, data);
    }).catch(function(e) {
        panel.innerHTML = _entHeader() + '<div class="empty-state"><div class="empty-icon">'+icon('warn')+'</div><div class="empty-text">Error: ' + e + '</div></div>';
    });
}
...
function renderEntities(panel, data) {
    const vehs = Array.isArray(data.vehicles) ? data.vehicles : [];
    const peds = Array.isArray(data.peds)     ? data.peds     : [];

    const vehRows = vehs.map(function(v) {
        return '<tr>'
            ...
            + '<td style="text-align:right"><button class="btn btn-danger btn-xs" data-ca-action="entDelete" data-net="' + escNum(v.netId || 0) + '">'+icon('trash')+'</button></td>'
            + '</tr>';
    }).join('');

    const pedRows = peds.map(function(p) {
        return '<tr>'
            ...
            + '<td style="text-align:right"><button class="btn btn-danger btn-xs" data-ca-action="entDelete" data-net="' + escNum(p.netId || 0) + '">'+icon('trash')+'</button></td>'
            + '</tr>';
    }).join('');

    const vehSection = vehs.length
...
caAction('entDelete', (d) => deleteEntityByNet(Number(d.net)));
